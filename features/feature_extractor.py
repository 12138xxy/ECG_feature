...
class Features:
    ...
    def _apply_filter(self, signal_raw, filter_bandwidth):
        ...
        # Check kurtosis for each mode and decide whether to denoise
        denoised_modes = []
        for mode in modes:
            mode_kurtosis = kurtosis(mode)
            if mode_kurtosis > 3.5:
                # Apply denoising to the mode if kurtosis exceeds threshold
        ...
        # Combine denoised modes into a processed signal
